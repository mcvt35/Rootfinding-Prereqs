...
class ContentFilter(object):   
    ...
    def __init__(self, filename):
        ...
        self.alpha_chars = sum([s.isalpha() for s in self.contents]) 
        self.numer_chars = sum([s.isdigit() for s in self.contents])   
        self.white_chars = sum([s.isspace() for s in self.contents])
        self.num_lines = 0
