...
def shut_the_box(player, timelimit):
    ...
    while time.time() < start_time + timelimit:
        print("\n")
        p_input = 0
        if sum(remaining_nums) <=6:
            rolls = random.randint(1, 6)
        else:
            rolls = random.randint(2, 12)
        print("Numbers left:", remaining_nums)
        print("Roll:", rolls)
        if not box.isvalid(rolls, remaining_nums):
            break
        ...
        while len(input_lst) == 0 or  not sum(input_lst) == rolls:
            print("Invalid input\n")
            print("Seconds left:", round(start_time + timelimit - time.time(), 2))
            p_input = input("Numbers to eliminate:")
            input_lst = box.parse_input(p_input, remaining_nums)
    ...
    if not winflag:
        ...
        print("YOU WON!\n")
    print("Points for player", player + ":", sum(remaining_nums), "points")
