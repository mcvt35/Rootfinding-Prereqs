...
def count_sets(cards):
    ...
    dupes = []
    if len(cards) != 12:
        raise ValueError("List must contain exactly 12 cards")
    for card in cards:
        if card in dupes:
            raise ValueError("List cannot contain repeat cards")
        else:
            dupes.append(card)
        if len(card) != 4:
            raise ValueError("Cards must have exactly four digits")
        for i in range(4):
            goodchars = ['0', '1', '2', '3']
            if card[i] not in goodchars:
                raise ValueError("Cards can only contain '0', '1', or '2'")
    total =[]
    for i in range(len(cards) - 2):
        for j in range(i + 1, len(cards) - 1):
            for k in range(i + 2, len(cards)):
                total.append(is_set(cards[i], cards[j], cards[k]))
